...
export default async function UserProfilePage({ params }) {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    return <main style={{ padding: 24 }}>Invalid user id.</main>;
  }
      ...
      <section>
        ...
        {user.events.length === 0 ? (
          ...
          <ul>
            {user.events.map((ev) => (
              <li key={ev.id}>
                {ev.title} <small style={{ color: '#666' }}>({fmt(ev.startAt)})</small>
              </li>
            ))}
          </ul>
        )}
          ...
          <ul>
            {user.comments.slice(0, 10).map((c) => (
              <li key={c.id}>
                ...
                <small style={{ color: '#666' }}>({fmt(c.createdAt)})</small>
              </li>
            ))}
          </ul>
        )}
          ...
          <ul>
            {user.rsvps.map((r, i) => (
              <li key={i}>
                ...
                <small style={{ color: '#666' }}>({fmt(r.event.startAt)})</small>
              </li>
            ))}
          </ul>
        )}
